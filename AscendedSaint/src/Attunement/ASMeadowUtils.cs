...
public static class ASMeadowUtils
{
    ...
    /// </summary>
    public static void ApplyMeadowHooks() => On.GameSession.ctor += MeadowGameSessionHook;

    /// <summary>
    /// Removes all Meadow-specific hooks from the game.
    /// </summary>
    public static void RemoveMeadowHooks() => On.GameSession.ctor -= MeadowGameSessionHook;
    ...
    internal static void MeadowGameSessionHook(On.GameSession.orig_ctor orig, GameSession self, RainWorldGame game)
    {
        if (OnlineManager.lobby is null || OnlineManager.lobby.isOwner)
        {
            ASLogger.LogDebug($"Player is host or in an offline session; Redirecting to default hook.");
        ...
        OnlineManager.lobby.owner.InvokeOnceRPC(typeof(ASRPCs).GetMethod("RequestRemixSync").CreateDelegate(typeof(Action<OnlinePlayer>)), OnlineManager.mePlayer);
    }
