...
public static class ASUtils
{
    ...
    public static void AscendCreature(Creature creature, Player callingPlayer)
    {
        ...
        if (creature.dead)
        {
            ...
            creature.Stun(creature is Player ? 60 : 120);
        }
        else
        {
            ...
            if (Utils.IsMeadowEnabled())
            {
                ...
                {
                    ASMeadowUtils.LogSystemMessage($"{ASMeadowQueries.GetPlayerName(player)} self-ascended.");
                }
            }
            else
            {
    ...
    public static void AscendCreature(PhysicalObject physicalObject, Player callingPlayer)
    {
        ...
        {
            ASLogger.LogInfo($"Return, Iterator! {Utils.GetOracleName(oracle.ID)}");
        ...
        else if (physicalObject is Creature creature)
        {
            AscendCreature(creature, callingPlayer);
        }
        else
        {
    ...
    public static bool CanReviveCreature(PhysicalObject physicalObject)
    {
        ...
        else if (physicalObject is Oracle oracle)
        {
            ...
            return ClientOptions.customOracleRevival && !oracle.Consious;
        }
    ...
    internal static void RemoveFromRespawnsList(Creature creature)
    {
        ...
        ASLogger.LogDebug($"Removed {creature} from the respawns list.");
    }
    ...
    internal static void ReviveOracle(Oracle oracle)
    {
        ...
        if (room.game.session is not StoryGameSession storyGame) return;
    ...
    public static void SpawnAscensionEffects(PhysicalObject physicalObject, bool isRevival = true)
    {
        ...
        ASLogger.LogDebug($"Spawned {(isRevival ? "revival" : "ascension")} effects at {pos} for {physicalObject}.");
    }
