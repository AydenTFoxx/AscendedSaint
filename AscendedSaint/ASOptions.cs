...
public class ASOptions : OptionInterface
{
    ...
    public class ClientOptions
    {
        ...
        public bool customOracleRevival;
        public bool dynamicEndings;
        ...
        public void RefreshOptions()
        {
            ...
            customOracleRevival = CUSTOM_ORACLE_REVIVAL.Value;
            dynamicEndings = DYNAMIC_ENDINGS.Value;
        }
        ...
        {
            allowSelfAscension = options.allowSelfAscension;
            allowRevival = options.allowRevival;
            requireKarmaFlower = options.requireKarmaFlower;
            revivalHealthFactor = options.revivalHealthFactor;
            customOracleRevival = options.customOracleRevival;
            dynamicEndings = options.dynamicEndings;
        }

        /// <summary>
        /// Obtains a representation of the client's REMIX options as a formatted <c>String</c>.
        /// </summary>
        /// <returns>A formatted <c>String</c> object with the client's current settings.</returns>
        protected string FormatOptions() => $"[AR: {allowRevival}; ASA: {allowSelfAscension}; RKF: {requireKarmaFlower}; RHF: {revivalHealthFactor} | COR: {customOracleRevival} | DE: {dynamicEndings}]";

        /// <summary>
        /// Obtains a <c>String</c> representation of the current <c>ClientOptions</c> instance.
        /// </summary>
        /// <returns>A <c>String</c> object with the client's current settings.</returns>
        public override string ToString() => $"ASOptions.ClientOptions => {FormatOptions()}";
    }
    ...
    public ASOptions()
    {
        ...
        CUSTOM_ORACLE_REVIVAL = config.Bind("custom_oracle_revival", false, new ConfigurableInfo("If enabled, Iterators other than FP and LttM can revived. Untested and prone to bugs."));
        DYNAMIC_ENDINGS = config.Bind("dynamic_endings", true, new ConfigurableInfo("Enables the mod's alternate endings for Saint."));
    }

    public override void Initialize()
    {
        ASLogger.LogInfo("ASOptions: Initialized REMIX menu interface.");
        base.Initialize();
    ...
    internal class OptionBuilder
    {
        ...
        public OptionBuilder(OptionInterface owner, string tabName, Color colorButton = default)
        {
            ...
            opTab.AddItems(
                [
                    new OpLabel(new Vector2(200f, 520f), new Vector2(200f, 40f), AscendedSaintMain.PLUGIN_NAME, bigText: true),
                    new OpLabel(new Vector2(200f, 505f), new Vector2(200f, 15f), "v" + AscendedSaintMain.PLUGIN_VERSION)
                    {
                        color = Color.gray
                    }
                ]
