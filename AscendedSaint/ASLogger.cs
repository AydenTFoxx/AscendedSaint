...
internal static class ASLogger
{
    ...
    public static void Log(LogLevel logLevel, object message, bool useUnityLogger = true)
    {
    ...
    private static string BuildLogPrefix(string acronym, bool isMeadowEnabled) => $"{acronym + (isMeadowEnabled ? "+M" : "")}";
    ...
    private static string FormatMessage(object message, LogLevel logLevel, bool addNewLine = true, bool addDateTime = true) =>
        $"{(addDateTime ? GetDateTime() : "")} [{BuildLogPrefix("AS", IsMeadowEnabled())}: {logLevel}] {message}".Trim() + (addNewLine ? Environment.NewLine : "");
